...
function findPersonalOSRoot(): string {
  const possiblePaths = [
...
function getObservations(limit: number = 5): string {
  const content = safeReadFile(PATHS.observations);
  if (!content) return "No observations logged yet.";
...
function getLatestEpisode(): string {
  try {
...
function logObservation(content: string): string {
  try {
...
function createEpisode(title: string, content: string): string {
  try {
...
function addTask(taskText: string, priority?: string): string {
  try {
...
function completeTask(taskText: string): string {
  const searchPaths = [
    ...
    tools: [
      {
        name: "get_goals",
        description:
          "Get current goals and objectives from Memory/goals.md. Use this to understand what Michael is working toward.",
        inputSchema: {
          type: "object",
          properties: {},
          required: [],
      ...
      },
      {
        name: "get_identity",
        description:
          "Get the AI assistant identity and philosophy from Memory/identity.md.",
        inputSchema: {
          type: "object",
          properties: {},
          required: [],
      ...
      },
      {
        ...
        description:
          "Get recent observations and patterns from Memory/observations.md. These are logged insights about ADHD patterns, resistance, wins, etc.",
        inputSchema: {
          type: "object",
          properties: {
            limit: {
      ...
      },
      {
        ...
        description:
          "Get the most recent episode log from Memory/episode_logs/. Episodes capture session context and what happened.",
        inputSchema: {
          type: "object",
          properties: {},
          required: [],
      ...
      },
      {
        name: "get_today_plan",
        description:
          "Get today's plan from the active task files. Shows scheduled blocks and tasks for today.",
        inputSchema: {
          type: "object",
          properties: {},
          required: [],
      ...
      },
      {
        name: "get_tasks",
        description:
          "Get unchecked tasks from today's plan. Quick way to see what needs to be done.",
        inputSchema: {
          type: "object",
          properties: {},
          required: [],
      ...
      },
      {
        ...
        description:
          "Log an observation, pattern, or insight to Memory/observations.md. Use this to capture ADHD patterns, resistance moments, wins, or learnings.",
        inputSchema: {
          type: "object",
          properties: {
            content: {
      ...
      },
      {
        name: "create_episode",
        description:
          "Create a new episode log in Memory/episode_logs/. Use this to capture session context, what was discussed, and where we left off.",
        inputSchema: {
          type: "object",
          properties: {
            title: {
      ...
      },
      {
        name: "add_task",
        description:
          "Add a new task to Today.md. Quick capture for tasks that come up during conversation.",
        inputSchema: {
          type: "object",
          properties: {
            task: {
      ...
      },
      {
        name: "complete_task",
        description:
          "Mark a task as complete in the markdown files. Finds the task and checks it off.",
        inputSchema: {
          type: "object",
          properties: {
            task: {
